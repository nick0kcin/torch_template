...
class DLASeg(nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad():
            x = self.encoder(x)
        # t = x[-1].view(x[-1].shape[0], x[-1].shape[1], -1)
        # S = torch.matmul(t, t.transpose(1, 2))
        # m = S.view(x[-1].shape[0], -1).softmax(dim=0)
        m = x[-1].clone()
        x = self.dla_up(x)
    ...
    def get_deconv_params(self):
        params = []
            # if self.up:
            #     params += list(self.up.parameters())
        params += list(self.ida_up.parameters())
        # for head in self.heads:
        #     params += list(self.__getattr__(head).parameters())
        return params

    def get_deconv_params2(self):
        params = []
        # if self.up:
        #     params += list(self.up.parameters())
        # params += list(self.ida_up.parameters())
        params += list(self.dla_up.parameters())
        # for head in self.heads:
        #     params += list(self.__getattr__(head).parameters())
        return params
    ...
    def set_group_param(self, group):
        params_map = {
            0: self.get_head_params,
            1: self.get_deconv_params,
            2: self.get_deconv_params2,
            3: self.parameters
                      }
